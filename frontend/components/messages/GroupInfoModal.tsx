    ...
    <div className="fixed inset-0 z-50 bg-black/60 dark:bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 animate-fade-in">
      <div className="w-full max-w-md bg-white dark:bg-[#130E26] text-slate-800 dark:text-slate-100 border border-slate-200 dark:border-indigo-500/20 rounded-2xl overflow-hidden shadow-2xl flex flex-col max-h-[85vh]">
        ...
        <div className="p-4 flex-1 overflow-y-auto space-y-4">
          ...
          {/* Group details */}
          <div className="flex flex-col items-center text-center pb-3 border-b border-slate-200/80 dark:border-indigo-500/10">
            <div className="w-16 h-16 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-2xl font-bold text-white shadow-lg border border-slate-200 dark:border-indigo-500/30 mb-2">
            ...
            <h3 className="font-bold text-lg text-slate-900 dark:text-white">{conversation.name}</h3>
          ...
          <div>
            ...
            {showAddMembers && (
              <div className="p-3 bg-slate-50 dark:bg-black/20 rounded-xl border border-slate-200 dark:border-indigo-500/15 mb-3 space-y-2">
                      ...
                      return (
                        ...
                        >
                          ...
                          <input
                            ...
                            checked={isSelected}
                            onChange={() => {}}
                            className="rounded border-slate-300 dark:border-indigo-500/30 text-indigo-600"
                          />
                        </div>
                      );
                    })
            ...
            <div className="space-y-1.5 max-h-48 overflow-y-auto pr-1">
              {conversation.members.map((member) => {
                ...
                return (
                  <div
                    key={member.userId}
                    className="flex items-center justify-between p-2 rounded-xl bg-slate-50 dark:bg-black/10 hover:bg-slate-100 dark:hover:bg-white/5 border border-slate-200/60 dark:border-indigo-500/5 transition-colors"
                  >
                    <div className="flex items-center gap-2.5">
                      <div className="relative w-8 h-8 rounded-full overflow-hidden border border-slate-200 dark:border-indigo-500/20">
                        {member.avatarUrl ? (
                          <img src={member.avatarUrl} alt={member.username} className="w-full h-full object-cover" />
                        ) : (
                          <div className="w-full h-full bg-indigo-600/30 text-indigo-500 dark:text-secondary text-xs font-bold flex items-center justify-center">
          ...
          {/* Leave group button */}
          <div className="pt-2 border-t border-slate-200/80 dark:border-indigo-500/10">
            <button
