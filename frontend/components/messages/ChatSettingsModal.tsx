      ...
      <div className="w-full max-w-md bg-white dark:bg-[#130E26] text-slate-800 dark:text-slate-100 border border-slate-200 dark:border-indigo-500/20 rounded-2xl overflow-hidden shadow-2xl flex flex-col max-h-[85vh]">
          ...
          <div className="flex items-center justify-between pt-2">
            <div>
              <div className="text-sm font-semibold text-slate-800 dark:text-slate-100 flex items-center gap-2">
                <span>🔔</span> Âm thanh thông báo
              </div>
              <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                Phát âm thanh chuông khi có tin nhắn mới đến
              </p>
            </div>
          ...
          <div className="flex items-center justify-between pt-3">
            ...
            {notificationPermission === 'granted' ? (
              ...
              <button
                ...
                className="px-3 py-1.5 bg-indigo-500/15 hover:bg-indigo-500/25 text-indigo-600 dark:text-indigo-400 rounded-lg text-xs font-semibold border border-indigo-500/30 whitespace-nowrap ml-3"
              >
                Cấp quyền
              </button>
            )}
        ...
        <div className="p-3 bg-slate-50/50 dark:bg-black/20 border-t border-slate-200/80 dark:border-indigo-500/10 flex justify-end">
          <button
            type="button"
            onClick={onClose}
            className="px-5 py-2 btn-anime text-xs font-bold rounded-xl shadow-md"
          >
            Đóng
          </button>
        </div>
