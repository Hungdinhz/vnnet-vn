...
def send_request(friend_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    request =  crud_friend.send_friend_request(db, user_id=current_user.id, friend_id=friend_id)
    return request
...
def accept_request(request_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    request = crud_friend.accept_friend_request(db, request_id=request_id, current_user_id=current_user.id)
    return request
